...
bool Cilindro::intersecta(const Rayo& rayo, float& alfa) const {
    alfa = 1e20f; // Inicializamos con un valor infinito (ver esto q tal)
    ...
    if (std::abs(rayo.direccion.y) > 1e-6f) {
        ...
        if (alfaBase > 0.001f && alfaBase < alfa) {
        ...
        if (alfaTop > 0.001f && alfaTop < alfa) {
